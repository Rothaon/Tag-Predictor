...
namespace TaggerPred
{
    public partial class Form1 : Form
    {
        ...
        public int PicBufferSize = 2; //Cuando baja de esto cargamos mas
        public int PicInitialBuffer = 10; //Las que cargamos al principio
        public static int PicBatchSize = 10;//Las que cargamos cada vez
        ...
        //Se actualizan los datos segun las opciones al pedir datos a la api
        ...
        public void UpdateRatioList(string tags)
        {
            ...
            foreach (KeyValuePair<string, float> entry in TagRatio)
            {
                ...
                lb_Score.Items.Add(string.Format("{0:00.00} {1} {2} de {3}", 100 * entry.Value, entry.Key, TagVotes[entry.Key], TagCount[entry.Key]));
            }
            lb_Score.EndUpdate();
        }
        ...
        public void ChangePic(int vote)
        {
            if (vote > -2)
            {
                ...
                {
                    //if (tag.Contains("&") || tag.Contains(">") || tag.Contains("<")) { continue; }//Los & Joden el xml
            ...
            //Si vote == -3 es la primera carga. Entonces no hay que borrar la imagen de la lista
            if (vote > -3 && PicturesToLoad.Count > 0)
            {
                PicturesToLoad.RemoveAt(0); //Borramos
            }

            //Despues de guardar todos las cosas relacionadas con los datos de la imagen:
            SaveVotes();
            //Comrpobamos cuantas quedan:
        ...
        public void GetPicUrls()
        {
            ...
            {//Evita que meta numero al principio
                TaggerOpt.LowestLiked.Add(LowestLikedBatch);
            }
            ...
            //Quitar las que estan prohibidas. Las prohibidas tienen prob = -1
            ...
            //Reordenar
            //PicturesToLoad.OrderBy<PicData1,float>(pic => pic.Likeliness);
        ...
        public void DownloadPic(PicData1 pic)
        {
            ...
            Client.DownloadFile(pic.DownloadUrl, TaggerOpt.DownloadFolder + pic.PicId + "." + foo.Last());//@"C:\folder\stackoverflowlogo.png");
